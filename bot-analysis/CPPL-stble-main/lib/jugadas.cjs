...
// ============================================
// 🆕 MONGODB (con fallback a archivos)
// ============================================
...
// 🆕 MongoDB con caché síncrono en memoria
// ============================================
...
function guardarUsuariosAcceso(usuarios) {
    // Guardar en MongoDB (caché + persistencia)
...
function cargarUltimaLoteria(userId) {
    ...
    try {
        const datos = db.obtenerUserData(userId, 'ultima_loteria');
        if (datos && datos.loteria) return datos.loteria;
    } catch (e) {}
...
function guardarUltimaLoteria(userId, loteria) {
    // 🆕 Guardar en MongoDB
    try {
        db.guardarUserData(userId, 'ultima_loteria', { loteria });
    } catch (e) {}
...
function cargarConfiguracionPremios(userId, loteria) {
    ...
    // 🆕 Intentar MongoDB primero
    try {
        const datos = db.obtenerUserData(userId, `config_premios_${loteria.toLowerCase()}`);
        if (datos) return datos;
    } catch (e) {}
...
function guardarConfiguracionPremios(userId, config, loteria) {
    ...
    // 🆕 Guardar en MongoDB
    try {
        db.guardarUserData(userId, `config_premios_${loteria.toLowerCase()}`, config);
    } catch (e) {}
...
function cargarListerosPorciento(userId, loteria) {
    ...
    // 🆕 Intentar MongoDB primero
    try {
        const datos = db.obtenerUserData(userId, `porcientos_${loteria.toLowerCase()}`);
        if (datos) {
            const nuevoFormato = {};
...
function guardarListerosPorciento(userId, listeros, loteria) {
    ...
    // 🆕 Guardar en MongoDB
    try {
        db.guardarUserData(userId, `porcientos_${loteria.toLowerCase()}`, listeros);
    } catch (e) {}
...
function procesarMensajeConContexto(userId, texto) {
    ...
    texto = texto.replace(/(\d{2}) *al *(\d{2}) *(com|con|c|cu|cdu) *(\d{1,4})/gi, (m, p1, p2, p3, p4) => `${parseInt(p1).toString().padStart(2, '0')}al${parseInt(p2).toString().padStart(2, '0')}-${p4}`);

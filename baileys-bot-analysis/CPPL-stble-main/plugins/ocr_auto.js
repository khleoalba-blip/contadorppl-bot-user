...
function generarTike(userId, listeros, premios, detalles, totalesListeros, pick3, pick4) {
    ...
    // Función para formatear números con separador de miles (ej: 3600 -> 3,600)
    const fmt = (num) => Math.round(num || 0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");

    // Función para asignar el icono correcto según la lotería
    const getLoteriaDisplay = (lot) => {
        ...
        return `🎰 ${lot}`; // Icono por defecto por si acaso
    };
    ...
    let tike = `\`\`\` ${fechaStr} \`\`\`\n\n`;
    tike += `> \`\`\`ContadorPPL\`\`\` 🪀\n\n`;
    tike += `\`${getLoteriaDisplay(loteria)}\`: *${pick3 || '000'}* • *${pick4 || '0000'}*\n`;
    tike += `______________________\n`;

    // LÓGICA 1: Listeros CON premio
    for (const [listero, tipos] of Object.entries(premios)) {
        ...
        tike += `👤 *${listero}*\n`;
        tike += `💰 REC: *${fmt(totalJugado)} $* \n`;
        tike += `       - ${porcentaje}%: *${fmt(montoPorcentaje)} $* \n`;
        tike += `______________________\n`;
        tike += `🏆 PREM: *${fmt(totalListero)} $*\n\n`;

        for (const [tipo, monto] of Object.entries(tipos)) {
            const montosVistos = new Set();
            let montoJugado = 0;
            if (detalles[listero] && detalles[listero][tipo]) {
            ...
            // Imprimir detalles
            if (detalles[listero] && detalles[listero][tipo]) {
                for (const det of detalles[listero][tipo]) {
                    tike += `- \`${det}\`\n`;
                }
            }
            tike += `\n`;
        ...
        tike += `______________________\n`;
        tike += `🧮 *${fmt(totalListero)}-${fmt(montoPorcentaje)} :*\n\n`;

        if (balance > 0) {
            tike += ` 🫵 *DEUDA*\n\n`; // Cuando debes
            tike += ` 💸*${fmt(Math.abs(Math.round(balance)))} $*\n`;
        } else if (balance < 0) {
            tike += ` 🥳 *FONDO*\n\n`; // Cuando ganas
            tike += ` 💰*${fmt(Math.abs(Math.round(balance)))} $*\n`;
    ...
    for (const [listero, tipos] of Object.entries(listeros)) {
        ...
        const balance = montoPorcentaje - 0; // No hay premio

        tike += `👤 *${listero}*\n`;
        tike += `💰 REC: *${fmt(totalJugado)} $* \n`;
        tike += `       - ${porcentaje}%: *${fmt(montoPorcentaje)} $* \n`;
        tike += `______________________\n`;
        tike += `🏆 PREM: *👌Sin premio* \n\n`;
        
        tike += `______________________\n`;
        tike += `🧮 *0-${fmt(montoPorcentaje)} :*\n\n`;

        if (balance > 0) {
            tike += ` 🫵 *DEUDA*\n\n`; // Cuando debes
            tike += ` 💰*${fmt(Math.round(montoPorcentaje))} $*\n`;
        } else {
            tike += ` ⚖️ *NEUTRO*\n\n`;
            tike += ` 💰*0 $*\n`;
        }
...
async function procesarBuffer(userId, conn) {
    ...
    // Arrays para comparar crudo vs corregido
    const textosCrudos = [];
    const textosCorregidos = [];
    ...
    for (let i = 0; i < archivos.length; i++) {
        ...
        const crudo = textoOcr.trim();
        const corregido = corregirTextoOcr(textoOcr).trim();

        textosCrudos.push(crudo);
        textosCorregidos.push(corregido);

        if (typeof procesarMensajeConContexto !== 'function') continue;

        const resultado = procesarMensajeConContexto(userId, corregido);
    ...
    // =============================================
    // ENVIAR TEXTO OCR AL CHAT (un solo bloque)
    // Si son iguales → muestra el texto
    // Si son diferentes → solo muestra el corregido
    // =============================================
    if (primerM && textosCrudos.length > 0) {
        let msgOcr = '';
        const hayDiferencia = textosCrudos.some((crudo, idx) => crudo !== textosCorregidos[idx]);

        if (hayDiferencia) {
            // Solo muestra el corregido
            msgOcr += `🔍 *Texto corregido por OCR:*\n\n`;
        } else {
            // Son iguales, muestra uno solo
            msgOcr += `🔍 *Texto detectado por OCR:*\n\n`;
        }

        // Decide cuál array usar: si hay diferencia usa corregidos, si no cualquiera
        const textosAMostrar = hayDiferencia ? textosCorregidos : textosCrudos;

        if (textosAMostrar.length === 1) {
            msgOcr += `\`\`\`\n${textosAMostrar[0]}\n\`\`\``;
        } else {
            for (let i = 0; i < textosAMostrar.length; i++) {
                msgOcr += `┌─ *Imagen ${i + 1}* ─┐\n`;
                msgOcr += `\`\`\`\n${textosAMostrar[i]}\n\`\`\`\n`;
                msgOcr += `└──────────────┘\n\n`;
            }
        ...
        msgOcr += `\n${'━'.repeat(30)}\n`;
        msgOcr += `⬇️ *Procesando jugadas...*`;

        if (msgOcr.length > 4000) {
            const partes = [];
            let parte = '';
            for (const linea of msgOcr.split('\n')) {
                if ((parte + linea + '\n').length > 4000) {
                    if (parte) partes.push(parte);
                    parte = linea + '\n';
                } else {
            ...
            for (const p of partes) {
                await primerM.reply(p);
                await new Promise(r => setTimeout(r, 300));
            }
        } else {
            await primerM.reply(msgOcr);
        }
